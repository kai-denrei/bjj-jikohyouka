...
- **2026-07-20 — Gerald verdict #6**: (1) tapping the bell-curve widget exposes an ugly blue focus box — suppress pointer-focus ring, keep keyboard ring. (2) HeatMap becomes a DIVERGING palette (content-owner override of single-hue rule): needs-work areas = low-saturation earth tones (orange/red family), strong areas = blue-green; low saturation, dark-mode appropriate. (3) DIMENSIONS DECISION (fills the proposal's blank = option (b) for three qualities): new first-class categories a) pressure, b) connection, c) mind games, with draft questions from Gerald's stems — "I can apply relaxed pressure that shuts down my training partner" / "I can connect to their hips/shoulders/center via their limbs" / "I can force/predict their next move" (his stem had "reliably" — the axis format makes the quantifier unnecessary). Existing mq_ pressure/connection drafts migrate into the new categories. PARKED for Gerald: do the three new categories' core questions join the sweep (15 → 18) or stay dashboard-only deep-dives? (Current build: sweep stays positional-only at 15.)
- **2026-07-20 — Gerald verdict #7 (visual landing page)**: replace the text intro with the bell curves as HERO + explanation under, annotated with example-grappler dots on the curves (teaches the overlap thesis before the assessment — a live version of his Belt Color Expectations reference chart). Six annotated dots at specific belt+percentile positions: (1) left of white curve — "total beginner"; (2) top ~5% of blue curve (right shoulder) — "Blue belt, wrestling background"; (3) black curve peak — "average black belt"; (4) black curve right tail — "competitor black belt"; (5) brown curve left tail — "older brown belt, out of shape"; (6) purple curve right tail — "18, training since age 5". The overlap payoff: dots 2 (top blue ~x43) and 5 (out-of-shape brown ~x42) land at the same ability; dot 6 (elite purple ~x67) reaches competitor territory. Curve params from scales.json ability_axis. DESIGN RISK: 6 labels on 390px without collision — likely needs leader lines or numbered-dot + legend-list pattern.
- **2026-07-21 — Gerald verdict #8 (landing copy + dot overlays)**: (1) hero → "All Models are Wrong, Some are useful, Belt Colors are only moderately so." (2) overlap micro-label → "Similar Ability, Different Belts". (3) REPLACE the 1–6 ability-ordered legend list with per-dot MODAL OVERLAYS: hovering (desktop) or pressing (mobile) a numbered dot reveals that grappler's label + ability position; add a subtle "tap a dot" hint since labels are no longer always-visible. (4) final explanation text (Gerald's, shipped with spelling fix assesment→assessment and 'r=29'→'r ≈ .29' for app consistency; the "explains 29% of actual skill" gloss is his framing — CONTROLLER FLAGGED as statistically off (r=.29 → ~8% variance, not 29%); left as his words pending his call). Full final text: "Belt Color is a loose indicator of ability, and self-rating in general is even more noisy ('r ≈ .29' means that self-assessment of skills typically only explains 29% of actual skill, e.g. humans are notoriously bad at self-awareness). Nonetheless this self-assessment test is meant to surface potential areas for work. When answering, picture the vast range of all the people you've trained with; the young prodigies, the octopus-like purple belt leg-locker addict, the slow-heavy older black belt, the tiny brown belt with impeccable technique, the flow-gods, the omg-so-heavy-wtf wizards able to redirect and pinpoint their weight effortlessly where it stalls you the most, etc. Think 'Where does my game start to struggle?', answer honestly, perhaps get some insights."
- **2026-07-21 — Gerald verdict #10 (intake → mat hours)**: intake relevance review. DROP belt color (only powered the results belt-stage lens; and asking belt contradicts the "ability > belt" thesis) and DROP gi/no-gi/both (unused; settles the gi/no-gi-split open question as "one profile"). KEEP years training + avg sessions/week; DERIVE estimated total mat hours = years_mid × sessions_mid × 1.5 h/session × 48 weeks/yr (4 weeks off), shown transparently with the assumption. Consequence handled: remove the belt-stage lens (BELT_LENS in ResultsPage), show estimated hours as results context instead. Back-compat: IntakeSchema strips extra keys so old sessions (with belt/style) still load. If Gerald later wants an experience-band "strong profile for your hours" lens, that's a follow-up.
